    ...
    <v-container>
        <v-card style="border-radius: 20px; width: 100%;">
            <v-card-text>
                ...
                <v-row>
                    <v-col cols="4">
                        <v-text-field label="شماره تماس" disabled v-model="user.phone">
                    ...
                    <v-row>
                        <v-col cols="4">
                            <v-autocomplete label="استان" append-icon="WMi-down-open" :items="getProvinces" @change="changeProvince(user.province_id)" item-value="id" item-text="name"
                                            v-model="user.province_id">
                                <v-icon slot="prepend"> WMi-map-marked</v-icon>
                            </v-autocomplete>
                        </v-col>
                        <v-col cols="4">
                            <v-autocomplete label="شهر" append-icon="WMi-down-open" :items="getCities(user.province_id)" item-value="id" item-text="name"
...
<script>
    ...
    export default {
        ...
        methods: {
            ...mapActions("Authentication", ["info", "updateInfo"]),
            ...mapActions("City", ["loadProvinces", "loadCities"]),
            async register() {
                let response = await this.updateInfo(this.user);
        ...
        async created() {
            ...
            this.images = this.user.certificates.filter(x => x.collection_name === 'user_certificates');
